    ...
    try:
        ...
        for a in soup.findAll('div', attrs={'data-post-type-id': "1"}):
            ...
            if view:
                ...
                views.append(det)
        df = pd.DataFrame(
            {'Title': titles, 'No. of answers': answers, 'No. of views': views, 'No. of votes': votes, 'Users': users,
             'Reputation': reputations, 'Time_stamp': timestamps, 'Summary': summarys})
        df.to_csv('Stack_exchange(Scraped_data).csv', mode='a', header=False, index=False, encoding='utf-8')
